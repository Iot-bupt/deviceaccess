 ...
 */
package com.bupt.deviceaccess.actors;
...
import com.bupt.deviceaccess.actors.service.ActorService;
...
public class ActorSystemContext {
    ...
    private JsonNode toBodyJson(ServerAddress server, ComponentLifecycleEvent event, Optional<Exception> e) {
        ObjectNode node = mapper.createObjectNode().put("com/bupt/deviceaccess", server.toString()).put("event", event.name());
        if (e.isPresent()) {
    ...
    private JsonNode toBodyJson(ServerAddress server, String method, String body) {
        return mapper.createObjectNode().put("com/bupt/deviceaccess", server.toString()).put("method", method).put("error", body);
    }
}
